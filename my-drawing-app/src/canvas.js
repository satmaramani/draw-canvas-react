...
const CanvasDrawingApp = () => {
  ...
  const selectedToolRef = useRef('pen'); // Stores the currently active tool ('pen' or 'brush')
  const brushSizeRef = useRef(5); // Stores the current brush size for the 'brush' tool
  ...
  const [canvasWidth, setCanvasWidth] = useState(800); // Fixed canvas width for the drawing area
  const [canvasHeight, setCanvasHeight] = useState(600); // Fixed canvas height for the drawing area
  ...
  const redrawCanvas = useCallback(() => {
    ...
    paths.forEach(path => {
      ...
      ctx.lineWidth = (path.tool === 'pen' ? 2 : path.size) / zoomLevel; // Pen is fixed 2px, brush uses path.size
      ctx.lineCap = 'round';       // Make line endings rounded
      ctx.lineJoin = 'round';      // Make line joins rounded
      ctx.stroke();                // Render the path
    });

    ctx.restore(); // Restore the canvas context state (removes the scaling transformation)
  ...
  const draw = useCallback((event) => {
    ...
    setPaths(prevPaths => {
        ...
        if (currentPathPoints.length >= 2) {
            ...
            ctx.lineWidth = (selectedToolRef.current === 'pen' ? 2 : brushSizeRef.current) / zoomLevel;
            ctx.lineCap = 'round';
            ctx.lineJoin = 'round';
  ...
  // This effect runs only once after the initial render because its dependencies (`startDrawing`, `draw`, `endDrawing`) are stable (memoized with useCallback)
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
  ...
  // Handler for enlarging/zooming the canvas content
    ...
    <div className="flex flex-col items-center p-4 min-h-screen bg-gray-100 font-sans antialiased">
      ...
      <div className="flex flex-wrap justify-center items-center gap-4 p-4 bg-white rounded-xl shadow-lg mb-8 max-w-4xl w-full">
        ...
        <button
          ...
          className={`px-6 py-3 rounded-xl font-semibold transition duration-300 ease-in-out transform hover:scale-105
        ...
        <button
          ...
          className="px-6 py-3 rounded-xl font-semibold bg-purple-600 text-white hover:bg-purple-700 transition duration-300 ease-in-out shadow-md transform hover:scale-105"
        >
        ...
        <button
          onClick={clearCanvas}
          className="px-6 py-3 rounded-xl font-semibold bg-red-500 text-white hover:bg-red-600 transition duration-300 ease-in-out shadow-md transform hover:scale-105"
        >
          Clear Canvas
        </button>
      </div>
