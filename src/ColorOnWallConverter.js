...
import { applyColorOnWallFilter } from './filters/ColorOnWallFilter';
import './GhibliImageConverter.css'; // Reuse the existing CSS

const ColorOnWallConverter = () => {
  ...
  const handleConvertImage = async () => {
    ...
    try {
      ...
      try {
        ...
        const response = await axios.post(`${API_URL}/filter/coloronwall`, formData, {
          responseType: 'blob',
        ...
        const dataUrl = await applyColorOnWallFilter(originalImage.file);
  ...
  const handleDownload = () => {
    ...
    link.download = 'color-on-wall.png';
  ...
  return (
    <div className="upload-container">
      <div className="ghibli-header">
        <h1 className="upload-title">Color on Wall Converter</h1>
        <p className="upload-subtitle">Transform your photos into vibrant wall art style</p>
      </div>
...
export default ColorOnWallConverter;